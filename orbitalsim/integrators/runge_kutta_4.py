...
class RungeKutta4Integrator:
    ...
    def step(bodies, positions, velocities, dt):
        ...
        k2_vel = calculate_accelerations(bodies, positions + 0.5 * dt * k1_pos)

        k3_pos = velocities + 0.5 * dt * k2_vel
        k3_vel = calculate_accelerations(bodies, positions + 0.5 * dt * k2_pos)

        k4_pos = velocities + dt * k3_vel
        k4_vel = calculate_accelerations(bodies, positions + dt * k3_pos)
