...
class RungeKutta4Integrator:
    ...
    def step(positions, velocities, GMs, dt):
        ...
        k1_pos = velocities
        k1_vel = calculate_accelerations(positions, GMs, velocities)

        k2_pos = velocities + 0.5 * dt * k1_vel
        k2_vel = calculate_accelerations(positions + 0.5 * dt * k1_pos, GMs, k1_vel)

        k3_pos = velocities + 0.5 * dt * k2_vel
        k3_vel = calculate_accelerations(positions + 0.5 * dt * k2_pos, GMs, k2_vel)

        k4_pos = velocities + dt * k3_vel
        k4_vel = calculate_accelerations(positions + dt * k3_pos, GMs, k3_vel)
